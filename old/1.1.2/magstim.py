...
class Magstim(object):
    ...
    def poke(self, silent=False):
        ...
        if silent and self._connected:
            ...
            self._processCommand(b'Q@', None, 3)
...
class Rapid(Magstim):
    ...
    def rTMSMode(self, enable, receipt=False):
        ...
        error,message = self._processCommand(b'[010' if enable else b'[000', 'instrRapid', 4)
    ...
    def setFrequency(self, newFrequency, receipt=False):
        ...
        if not error:
            updateError,currentParameters = self.getParameters()
            if not updateError:
                updateError,currentParameters = self._processCommand(b'D' + bytearray(str(int(currentParameters['rapidParam']['duration'] * currentParameters['rapidParam']['frequency'])).zfill(4),encoding='ascii'), 'instrRapid', 4)
                if updateError:
    ...
    def setNPulses(self, newNPulses, receipt=False):
        ...
        If receipt argument is False:
            None
        """
        error, message = self._processCommand(b'D' + bytearray(str(int(newNPulses)).zfill(4),encoding='ascii'), 'instrRapid', 4)
        #If we didn't get an error, update the other parameters accordingly
        if not error:
            updateError, currentParameters = self.getParameters()
            if not updateError:
                updateError, currentParameters = self._processCommand(b'[' + bytearray(str(int((currentParameters['rapidParam']['nPulses'] / currentParameters['rapidParam']['frequency']) * 10)).zfill(3),encoding='ascii'), 'instrRapid' if receipt else None, 4)
                if updateError:
    ...
    def setDuration(self, newDuration, receipt=False):
        ...
        If receipt argument is False:
            None
        """
        error, message = self._processCommand(b'[' + bytearray(str(int(newDuration)).zfill(3),encoding='ascii'), 'instrRapid', 4)
        if not error:
            updateError, currentParameters = self.getParameters()
            if not updateError:
                updateError, currentParameters = self._processCommand(b'D' + bytearray(str(int(currentParameters['rapidParam']['duration'] * currentParameters['rapidParam']['frequency'])).zfill(4),encoding='ascii'), 'instrRapid', 4)
